...
from tickets.views import create_ticket  # Import the view function directly
...
def test_ticket_list_only_shows_own_tickets(auth_client, create_user, create_other_user):
    """Test a user only sees tickets they created."""
    client = auth_client
    user = create_user
    my_ticket = Ticket.objects.create(title="My Ticket", description="Mine", created_by=user)
    other_ticket = Ticket.objects.create(title="Other Ticket", description="Not mine", created_by=create_other_user)
...
def test_create_ticket_view_direct_call(create_user):
    """Test the create_ticket view by calling the function directly, bypassing the test client."""
    user = create_user
