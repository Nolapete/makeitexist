...
def pantry_item_update(request, pk):
    ...
    return render(request, 'pantry/pantry_item_form.html', {
        'form': form,
        'title': 'Edit Item',
        'item': item
    })   
...
def api_barcode_scan(request):
    ...
    try:
        ...
        return JsonResponse({
            'found': False,
            'error': 'Item not found in your pantry'
        }, status=404)

@login_required
def location_list(request):
    locations = Location.objects.prefetch_related('storage_units')
    return render(request, 'pantry/location_list.html', {
        'locations': locations
    })   
...
def location_detail(request, pk):
    ...
    return render(request, 'pantry/location_detail.html', {
        'location': location,
...
def location_create(request):
    ...
    return render(request, 'pantry/location_form.html', {
        'form': form,
...
def location_delete(request, pk):
    ...
    return render(request, 'pantry/location_confirm_delete.html', {
        'location': location
    })   
...
def storage_unit_detail(request, pk):
    unit = get_object_or_404(
        StorageUnit.objects.select_related('location'),
        pk=pk
    )
    stocks = unit.stocks.select_related('item').all()
    return render(request, 'pantry/storage_unit_detail.html', {
        'unit': unit,
        'stocks': stocks
    })   
...
def storage_unit_update(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            messages.success(request, f'✅ Updated {unit.get_unit_type_display()} "{unit.name}".')
    ...
    return render(request, 'pantry/storage_unit_form.html', {
        'form': form,
        'title': f'Edit {unit.get_unit_type_display()}',
        'unit': unit
    })   
...
def storage_unit_delete(request, pk):
    ...
    return render(request, 'pantry/storage_unit_confirm_delete.html', {
        'unit': unit
    })   
...
def stock_add(request):
    ...
    if request.method == "POST":
        ...
        messages.success(request, f"✅ Added {quantity} of '{item.name}' to '{unit.name}'.")
        return redirect('pantry:storage_unit_detail', pk=unit.pk)
...
def render_stock_add_form(request, item=None, unit=None, quantity='', expiry=''):    
    ...
    return render(request, 'pantry/stock_add.html', {
        'items': items,
        'units': units,
        'item': item,
        'unit': unit,
...
def stock_delete(request, pk):
    ...
    return render(request, 'pantry/stock_confirm_delete.html', {
        'stock': stock
    })   
            ...
            try:
                ...
                return JsonResponse({
                    ...
                    'item': {
                        ...
                        } if item.default_storage else None
                    }
                })
            except PantryItem.DoesNotExist:
                return JsonResponse({
...
def alerts_dashboard(request):
    ...
    expiring_soon = Stock.objects.filter(        
        ...
        quantity__gt=0
    ).select_related('item', 'storage_unit').order_by('expiry_date')

    # 2. Out of Stock (quantity == 0)
    out_of_stock = PantryItem.objects.filter(        
        stocks__quantity=0
    ).distinct().prefetch_related('stocks__storage_unit')

    # 3. Low Stock (quantity > 0 but below min_stock_level)
    low_stock = PantryItem.objects.exclude(
        stocks__quantity__gte=F('min_stock_level')  # quantity >= min_stock_level
    ).exclude(
        stocks__quantity=0  # already covered above
    ).distinct().prefetch_related('stocks__storage_unit')
